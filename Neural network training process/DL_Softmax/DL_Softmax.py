...
for i in range(iteration):
    ...
    y_p=Softmax_For(z_2)
    L= loss_For(train_data_lab,y_p)
    print("Loss:",L)
    Loss.append(L)
    dEdz_2=Softmax_Back(y_p,train_data_lab)
    ...
    for iii in range(valid_data_lab.shape[0]):
        if np.argmax(v_p[iii,:])==np.argmax(valid_data_lab[iii,:]):              # Compare the Prediction and validation
            a=a+1.00
...
t_2=InnerProduct_For(Sigmoid_For(t_1),w_out,b_out)
test_Out_data=Softmax_For(t_2)
